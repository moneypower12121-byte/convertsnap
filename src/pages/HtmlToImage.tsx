...
export default function HtmlToImage() {
  ...
  const jsonLd = {
    ...
    "description": "Turn HTML code into PNG or JPG images instantly. Free tool for OG images, email headers, social cards.",
  ...
  const handleConvert = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const pixelRatio = scale === '2x' ? 3 : 2; // Boost pixel ratio for crisper output
      ...
      // We wait a tiny bit to make sure any layout inside the hidden container is settled
      await new Promise(res => setTimeout(res, 100));

      const dataUrl = format === 'PNG' 
        ? await toPng(container, config)
        : await toJpeg(container, config);
        
      setGeneratedImageUrl(dataUrl);
      setStatus('success');
    } catch (err) {
      console.error('Error generating image:', err);
      setErrorMessage('Failed to generate image. Ensure your HTML is valid and external resources allow CORS.');
      setStatus('error');
    }
  };
  ...
  return (
    <>
      <Helmet>
        <title>HTML to Image Converter — Free HTML to PNG/JPG | ConvertSnap</title>
        <meta name="description" content="Convert HTML and CSS code to PNG or JPG image free online. Perfect for Open Graph images, email templates, social media cards. Try ConvertSnap HTML to Image." />
        <meta name="keywords" content="html to image, html to png, html to jpg, html css to image, convert html to screenshot, og image generator, social media card generator, email template to image" />
        <link rel="canonical" href="https://www.convertsnap.net/html-to-image" />
        
        <meta property="og:title" content="HTML to Image Converter — Free HTML to PNG | ConvertSnap" />
        <meta property="og:description" content="Turn HTML code into PNG or JPG images instantly. Free tool for OG images, email headers, social cards." />
        <meta property="og:url" content="https://www.convertsnap.net/html-to-image" />
              ...
              <div className="flex-1 p-6 md:p-9 border-b lg:border-b-0 lg:border-r border-border bg-white">
                {status !== 'success' ? (
                  ...
                  >
                    ...
                    {generatedImageUrl && (
                      <div className="mb-6 rounded-lg overflow-hidden border border-border shadow-sm max-h-48 max-w-full">
                        <img src={generatedImageUrl} alt="Generated Preview" className="max-h-48 object-contain" />
                      </div>
                    )}
              ...
              <div className="flex-1 bg-slate-50 p-6 md:p-9 flex flex-col">
                ...
                <div className="flex-grow flex items-center justify-center bg-white border border-border rounded-xl shadow-inner p-4 overflow-hidden relative" style={{ minHeight: '300px' }}>
                  ...
                  <div className="w-full flex items-center justify-center relative">
                    ...
                    >
                      <iframe 
                        srcDoc={htmlCode || '<html><body></body></html>'} 
